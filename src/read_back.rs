    ...
    match type_ {
        ...
        Type::Universe => match val {
            Value::PiType {
                ...
                let param_type = read_back_typed(defs, ctx, param_type, &Value::Universe);
                Expression::PiType {
                    tparam: fresh_id,
...
pub fn read_back_neutral(defs: &Definitions, ctx: &Context, neu: &Neutral) -> Expression {
    match neu {
        ...
        Neutral::Application { func, arg } => {
            ...
            let arg_type = match synth_type(defs, ctx, &func) {
